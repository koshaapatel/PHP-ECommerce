			...
			$connected = true;
				...
				if(sizeof($users)){
					if (password_verify($_POST['password'], $users[0]['password'])) {
							$sql = $db->prepare("UPDATE `tab_user` SET `password`= :password WHERE `tab_user`.`id` = :id");
							if($_POST['new_password'] == $_POST['confirm_password']){
								...
								$response->data = $_SESSION['uid'];
								$response->msg = "Password saved Successfully";
							}else{
								$response->status = false;
								$response->data = '';
								$response->msg = "New Password and Confirm Password must be same";
